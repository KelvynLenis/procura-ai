    ...
    <>
      <div className="flex flex-col w-full h-fit bg-white rounded-xl shadow-md">
        <div className="flex items-center justify-end w-full h-16 bg-primary rounded-t-xl px-4 gap-3">
          ...
          <button onClick={() => setIsAlertModalOpen(true)} className={cn("rounded-lg group relative w-10 h-10 ring-1 flex flex-col md:flex-row items-center justify-center", isRegular ? "ring-zinc-300 bg-white text-red-600 hover:bg-red-300 hover:ring-red-500" : "ring-red-700 text-white bg-red-600 hover:bg-red-100 hover:text-red-600")}>
            <IoIosWarning size={28} />
          </button>

          <button onClick={() => setIsEditModalOpen(true)} className="flex rounded-lg w-10 h-10 bg-white ring-1 ring-zinc-300 group relative hover:bg-sky-100 hover:ring-blue-700 hover:text-blue-900 items-center justify-center hover:opacity-90">
            <ImPencil size={20} />
          </button>
          {/* <Link href={`meus-dispositivos/edit/${id}`}>
          </Link> */}

          <button onClick={() => setIsDeleteModalOpen(true)} className="flex rounded-lg w-10 h-10 bg-white group relative items-center justify-center gap-2 ring-1 ring-zinc-300 hover:bg-red-200 hover:ring-red-600 text-red-600 hover:opacity-90">
            <Trash2 size={20} />
          </button>
        </div>
...

      {
        ...
        <Modal setModalOpen={setIsEditModalOpen} title="Editar dispositivo">
          <DeviceForm device={device} isPopover setModalOpen={setIsEditModalOpen} />
        </Modal>
