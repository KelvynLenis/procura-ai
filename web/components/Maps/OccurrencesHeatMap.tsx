...
export function OccurrencesHeatMap({ districts }: OccurrencesHeatMapProps) {
  ...
  function handleOverlayMouseOver({
    event,
    anchor,
    payload,
  }: { event: any; anchor: any; payload: any }) {
  ...
  function getFillColor(cod_neighborhood: number) {
    const district = districts.find(
      district => district.cod_neighborhood === cod_neighborhood
    )

    const robbery_counter = district?.robbery_counter
      ? district?.robbery_counter
      : 0
      ...
      >
        <GeoJsonLoader
          ...
          styleCallback={(feature, hover) =>
            hover
              ? {
                  fill: getHoverColor(Number(feature.properties.cod_bairro)),
                  opacity: 0.8,
                  strokeWidth: '2',
                  stroke: '#000',
                }
              : {
                  fill: getFillColor(Number(feature.properties.cod_bairro)),
                  opacity: 0.5,
                  strokeWidth: '1',
                  stroke: '#000',
                }
        ...
        <div className="absolute w-28 bottom-2 right-5 bg-black/50 py-2 px-4 rounded-md text-white">
          ...
          <ul className="flex flex-col gap-0">
            <li className="flex items-center gap-2 h-fit">
              <span className="w-6 h-5 p-1 bg-[#FECF3E] inline-block" />0 - 2
            </li>
            <li className="flex items-center gap-2">
              <span className="w-6 h-5 p-1 bg-[#F3AD39] inline-block" />2 - 3
            </li>
            <li className="flex items-center gap-2">
              <span className="w-6 h-5 p-1 bg-[#E78A33] inline-block" />4 - 5
            </li>
            <li className="flex items-center gap-2">
              <span className="w-6 h-5 p-1 bg-[#DC662E] inline-block" />6 - 20
            </li>
