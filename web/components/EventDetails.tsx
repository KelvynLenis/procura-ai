...
export function EventDetails({
  ...
  const [selectedOccurrence, setSelectedOccurrence] = useState<OccurrencesProps | null>(null)
  const [isViewingDetails, setIsViewingDetails] = useState(false)

  const fullScreenMap = pathname === 'map/ocorrencias'
  const hasMultipleOccurrences = sameLocationOccurrences && sameLocationOccurrences.length > 1
  ...
  const currentPageItems = hasMultipleOccurrences ? sameLocationOccurrences!.slice(startIndex, endIndex) : []
        ...
        <div className="min-h-[240px]">
          {currentPageItems.map((occ, index) => (
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="font-medium text-xs truncate">{occ.device.phone_model}</span>
                <span className="text-xs text-gray-500 truncate">{occ.device.brand}</span>
              </div>
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-gray-500 truncate">{formatType(occ.event.type)}</span>
              </div>
