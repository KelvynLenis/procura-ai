...
import { listAllEvents } from "./list-all-events";
...
describe("listAllEvents", () => {
  ...
  it("deve retornar eventos com sucesso", async () => {
    ...
    const result = await listAllEvents("device-123");
  ...
  it("deve utilizar o deviceId informado", async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        documents: [],
    ...
    await listAllEvents("device-123");

    const url = decodeURIComponent(vi.mocked(fetch).mock.calls[0][0] as string);

    expect(url).toContain("id_device");
    expect(url).toContain("device-123");
  });

  it("deve utilizar string vazia quando deviceId não for informado", async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        documents: [],
