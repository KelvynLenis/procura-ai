...
const ConfirmationDialog = ({ isModalVisible, setIsModalVisible, title, description, onConfirm }: ConfirmationDialogProps) => {
  ...
  return (
    <Modal animationType='fade' transparent visible={isModalVisible} onRequestClose={() => setIsModalVisible(false)}>
        <Pressable className='flex-1 bg-black/50 flex items-center justify-center' onPress={() => setIsModalVisible(false)}>
          <Pressable onPress={(e) => e.stopPropagation()} className='bg-white w-[90%] h-72 p-4 gap-5 flex rounded-2xl overflow-hidden'>
