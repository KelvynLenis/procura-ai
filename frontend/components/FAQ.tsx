...
export function FAQ() {
  ...
  return (
    <section className='w-full h-fit bg-faq flex justify-center pt-28 pb-10'>
      <div className="flex flex-col gap-5 w-full h-full px-32">
            ...
            questions.map((question, index) => (
              <Accordion key={index} type="single" collapsible>
                <AccordionItem className="bg-white rounded-xl text-secondary flex flex-col gap-0" value={`item-${index}`}>
                  ...
                  <AccordionContent className="px-10">{question.answer}</AccordionContent>
                </AccordionItem>
              </Accordion>
            ))
          }
